...
import re
text = re.sub(
    r'public Result<TaskRecord> generateDynamic\(@PathVariable Long projectId,[\s]*@AuthenticationPrincipal UserDetails userDetails\)\s*\{\s*Long userId = currentUserHelper\.getUserId\(userDetails\);\s*return Result\.success\(videoCompositionService\.startGenerateDynamicVideos\(userId,\s*projectId\)\);',
