...
# Strip any previously badly injected dialog at the end
clean_dialog_regex = r'<!-- Shot Selection Dialog -->\s*<el-dialog.*?</el-dialog>\s*'
text = re.sub(clean_dialog_regex, '', text, flags=re.DOTALL)
...
# Now inject right before </template>
text = re.sub(r'</template>', dialog_html + '\n</template>', text, count=1)

with codecs.open(fpath, 'w', 'utf-8') as f:
    f.write(text)
print("Added HTML correctly!")
