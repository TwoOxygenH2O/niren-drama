...
- ✅ 文生图：通义万相 Wanx 2.1、DashScope、OpenAI DALL-E、可灵 AI、即梦 AI、Stable Diffusion、**火山引擎 Seedream 3.0**（新增）
- ✅ 文生视频：可灵 AI、即梦 AI、Runway、**火山引擎 Seedance 2.0**（新增）
- ✅ TTS 语音：**阿里云 CosyVoice V3**（新增）、**科大讯飞星火 TTS**（新增）、火山引擎、OpenAI TTS、MiniMax
- ✅ 文本大模型：DeepSeek、通义千问、火山豆包、智谱 GLM、百川、文心一言、MiniMax、OpenAI 等
- ✅ 每类配置支持多个，可设置默认，配置数据库持久化（按用户隔离）
- ✅ 智能默认：选择服务商后自动填充 Base URL 与推荐模型
- ✅ 服务商提示：选中推荐服务商后显示开通指引与价格参考
...
| 类型 | 支持服务商 |
|------|-----------|
| **文本大模型** | OpenAI GPT 系列 / DeepSeek / 火山引擎豆包 / 阿里通义千问 / 百度文心 / 智谱 GLM / 百川 / MiniMax（所有 OpenAI 兼容接口均可接入） |
| **文生图** | 通义万相 Wanx 2.1（推荐）/ 阿里云百炼 DashScope / 火山引擎 Seedream 3.0 / OpenAI DALL-E 3 / 可灵 AI / 即梦 AI / Stable Diffusion |
| **文生视频** | 火山引擎 Seedance 2.0（推荐）/ 可灵 AI / 即梦 AI / Runway |
| **TTS 语音** | 阿里云 CosyVoice V3（推荐）/ 科大讯飞星火 TTS / 火山引擎 TTS / OpenAI TTS / MiniMax（内置多款音色 Mock，接入真实 TTS 只需实现 `TtsProvider` 接口） |
...
| 场景管理 | ✅ 完成 | ✅ 完成 | AI 生成背景图 |
| 素材库 | ✅ 完成 | ✅ 完成 | 上传/筛选/分页 |
| AI 配置 | ✅ 完成 | ✅ 完成 | 多厂商配置 + 推荐服务商（Wanx/Seedance/CosyVoice/讯飞等）|
| 视频合成 | ✅ 完成 | ✅ 完成 | FFmpeg 单镜头合成 + 整集拼接（竖屏 9:16 / 1080×1920） |
| 成片导出 | ✅ 完成 | ✅ 完成 | 竖屏 9:16 成片下载（导出后自动删除节约空间） |
| Docker 部署 | ✅ 完成 | ✅ 完成 | docker-compose 一键启动 |

> **整体进度：全部 10 个主要功能模块已完成，系统进入可商用阶段。**
...
- [x] v0.7 — 视频合成（FFmpeg 单镜头合成 + 整集拼接）
- [x] v0.8 — 成片导出（导出后自动删除节约空间）
